...
test('nearestLandmarkName returns null when nothing is within maxDist', () => {
  assert.equal(nearestLandmarkName(200, 200, LANDMARKS, HOME), null);
});

test('nearestLandmarkName names the closest landmark within range', () => {
  assert.equal(nearestLandmarkName(23, 5, LANDMARKS, HOME), 'the Split Oak');
  assert.equal(nearestLandmarkName(-94, 45, LANDMARKS, HOME), 'the Drowned Car');
});
...
test('nearestLandmarkName respects a custom maxDist', () => {
  assert.equal(nearestLandmarkName(30, 4, LANDMARKS, HOME, 5), null);
  assert.equal(nearestLandmarkName(30, 4, LANDMARKS, HOME, 50), 'the Split Oak');
});
...
test('nearestLandmarkName falls back to the raw kind for an unnamed landmark kind', () => {
  ...
  assert.equal(nearestLandmarkName(11, 10, unknown, HOME), 'mysteryStone');
});
...
test('formatChronicle appends a nearby landmark name when args.landmark is set', () => {
  const events: ChronicleEvent[] = [
    ...
    { t: 20, code: 'death', args: { kind: 'lion', landmark: 'the Split Oak' } },
  ];
  assert.deepEqual(formatChronicle(events), [
    '0:10 — a bear caught your scent near the Fire Tower.',
    '0:20 — a lion caught you near the Split Oak.',
  ]);
});
