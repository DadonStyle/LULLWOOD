...
test('holding from full charge activates immediately and drains at 1/VEIL_MAX_HOLD per second', () => {
  const s0: VeilChargeState = { charge: 1, locked: false, reserve: false };
...
test('not holding is never active, and charge regenerates toward 1', () => {
  const s0: VeilChargeState = { charge: 0.5, locked: false, reserve: false };
...
test('regen rate is VEIL_REGEN_MUL times the drain rate', () => {
  const drained = stepVeilCharge({ charge: 1, locked: false, reserve: false }, true, 1);
  const regened = stepVeilCharge({ charge: 0, locked: false, reserve: false }, false, 1);
...
test('charge never goes below 0 or above 1', () => {
  const over = stepVeilCharge({ charge: 0.01, locked: false, reserve: false }, true, 10);
  assert.equal(over.charge, 0);
  const under = stepVeilCharge({ charge: 0.99, locked: false, reserve: false }, false, 10);
...
test('a full continuous hold drains exactly to 0 over VEIL_MAX_HOLD seconds and locks', () => {
  const s: VeilChargeState = { charge: 1, locked: false, reserve: false };
...
test('once locked, holding the key does nothing -- veil is inactive even though held', () => {
  const locked: VeilChargeState = { charge: 0.1, locked: true, reserve: false };
  const s = stepVeilCharge(locked, true, 0.016);
  assert.equal(s.active, false);
});

test('a locked veil regenerates (does not get stuck at 0 forever)', () => {
  const s = stepVeilCharge({ charge: 0, locked: true, reserve: false }, true, 1); // still held, but locked -- should still regen
  assert.ok(s.charge > 0);
});

test('locked stays locked until charge crosses VEIL_UNLOCK_CHARGE, not the instant it leaves 0', () => {
  let s = stepVeilCharge({ charge: 0, locked: true, reserve: false }, true, 0);
...
test('unlocks and reactivates on the frame charge reaches VEIL_UNLOCK_CHARGE, if still held', () => {
  const justBelow: VeilChargeState = { charge: VEIL_UNLOCK_CHARGE - 0.001, locked: true, reserve: false };
  ...
  const atThreshold: VeilChargeState = { charge: VEIL_UNLOCK_CHARGE, locked: true, reserve: false };
...
test('releasing before a full drain never locks -- partial use is free to resume', () => {
  const partial: VeilChargeState = { charge: 0.4, locked: false, reserve: false };
...
test('maxHold defaults to VEIL_MAX_HOLD -- omitting it matches passing it explicitly', () => {
  const withDefault = stepVeilCharge({ charge: 1, locked: false, reserve: false }, true, 1);
  const explicit = stepVeilCharge({ charge: 1, locked: false, reserve: false }, true, 1, VEIL_MAX_HOLD);
...
test('a larger maxHold drains slower -- a Deeper Lungs tier makes the same hold cost less charge', () => {
  const base = stepVeilCharge({ charge: 1, locked: false, reserve: false }, true, 1, 5);
  const upgraded = stepVeilCharge({ charge: 1, locked: false, reserve: false }, true, 1, 8);
...
test('a full continuous hold at an upgraded maxHold drains exactly to 0 over that many seconds', () => {
  const result = stepVeilCharge({ charge: 1, locked: false, reserve: false }, true, 8, 8);
  assert.equal(result.charge, 0);
  assert.equal(result.locked, true);
});
...
test('Deeper Lungs (larger maxHold) does not extend the post-drain lockout -- lockout time stays ~3s across all tiers', () => {
  function lockoutSeconds(maxHold: number): number {
    let s: VeilChargeState = { charge: 0, locked: true, reserve: false };
    let t = 0;
    const dt = 0.001;
    while (s.locked && t < 60) {
...
// ---- stepVeilCharge: reserve (LUL-1210 Stone Marker charm) -----------------

test('a full drain with reserve:true snaps back to the unlock threshold instead of locking', () => {
  const s: VeilChargeState = { charge: 1, locked: false, reserve: true };
  const result = stepVeilCharge(s, true, VEIL_MAX_HOLD);
  assert.equal(result.locked, false);
  assert.equal(result.reserve, false);
  assert.ok(Math.abs(result.charge - VEIL_UNLOCK_CHARGE) < 1e-9);
});

test('a full drain with reserve:true at an upgraded maxHold snaps back to the scaled unlock threshold', () => {
  const s: VeilChargeState = { charge: 1, locked: false, reserve: true };
  const result = stepVeilCharge(s, true, 8, 8);
  assert.equal(result.locked, false);
  assert.equal(result.reserve, false);
  assert.ok(Math.abs(result.charge - (VEIL_UNLOCK_CHARGE * VEIL_MAX_HOLD / 8)) < 1e-9);
});

test('a full drain with reserve:false behaves exactly as today -- locks out', () => {
  const s: VeilChargeState = { charge: 1, locked: false, reserve: false };
  const result = stepVeilCharge(s, true, VEIL_MAX_HOLD);
  assert.equal(result.charge, 0);
  assert.equal(result.locked, true);
...
test('a partial drain that does not reach 0 leaves reserve untouched', () => {
  const s: VeilChargeState = { charge: 1, locked: false, reserve: true };
  const result = stepVeilCharge(s, true, 1);
