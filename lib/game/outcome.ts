...
// pickup, win, death, restart -- lifted out of five duplicated
...
// The engine keeps every mutable local (won, dead, pickingUp,
...
export function freshRunState(): RunState {
  return { entered: false, won: false, dead: false, pickingUp: false, babyTaken: false };
}
...
function pickupAllowed(s: RunState): boolean {
  return !s.babyTaken && !s.won && !s.dead && !s.pickingUp;
}
...
/** Deliberately includes `pickingUp` in the guard -- death during the ~11.3s
 * pickup cinematic is ignored on purpose, a pre-existing invulnerability
 * window this extraction must pin, not "fix". If it looks wrong, file a P3
 * and leave the behaviour alone (wiki systems/unit-testing-standard). */
 ...
 * on pickingUp -- regenerating mid-cinematic is a dev-tool footgun, not a
 * state-machine violation, so it's left alone (wiki systems/unit-testing-standard). */
 ...
 * not run-phase -- you can grab a throwable in any playable state. */
 ...
 * throwing while investigate/chase is active etc. is allowed. */
