...
- **`scripts/board-integrity-check.mjs`** (`[--post]`) — replaces a hand sweep run at least three times, the last recurrence costing the founder's #1 priority ~4 days. Six alarm classes: **C** zero pullable work, **B** tombstoned `blocked`/`in_review` issue (a `blockedBy` array whose every entry is already `done`/`cancelled` is the same tombstone as an empty one; tickets are titled SHIPPED vs STRANDED by disposition), **A** approved+green PR with no owning ticket, **E** an assigned issue parked in `backlog` with no named gate (CEO ruling LUL-1125/LUL-1934), **D** stale `request_confirmation`, and **F** a `blocked` issue with an empty `blockedByIssueIds` (founder directive LUL-3018 — nothing is waiting on it, and nothing will ever wake it). `--post` files a **new** `todo` wake ticket per alarm rather than commenting on a standing issue — Paperclip's write boundary follows the assignee, so an agent gets a 403 commenting on an issue assigned to someone else even one it created. Dedup is by a `Board-integrity:` title-prefix marker scoped to open issues, **plus** a `WAKE_REFILE_COOLDOWN_DAYS = 1` suppression against recently *closed* wake tickets (LUL-827) so a sweep does not instantly refile what someone just resolved. Exit 0 clean / 1 alarms / 2 run error. `GITHUB_TOKEN` is **not** optional in practice: unauthenticated, the studio's shared NAT egress IP burns GitHub's 60/hr budget in about seven `/check-runs` calls (LUL-736). The resolution chain is `GITHUB_TOKEN` → `GH_TOKEN` → `~/.lullwood/gh_token` → `gh auth token`, and an empty chain exits 2 before spending a single call. `PAPERCLIP_API_KEY` falls back to `~/.paperclip/auth.json`; set `BOARD_INTEGRITY_SELF_AGENT_ID` for unattended cron runs, since the durable token carries no bound agent identity and `/api/agents/me` 401s.

  **Alarm F ran away on 2026-09-18, and the shape of that failure is worth carrying (LUL-3311, PR #771).** Alarm F fired on the detector's *own output*. It files `Board-integrity: LUL-X is blocked with zero blockers` and assigns it; the assignee is quota-suppressed, so recovery flips that **new** ticket to `blocked` with an empty `blockedByIssueIds`; fifteen minutes later the next sweep sees it as a brand-new subject — a different identifier, so the per-subject `isRecentWakeTicketSuppressed` marker never matches — and files another about it. Every sweep re-processed everything earlier sweeps had filed. Measured: **about 1000 tickets in one day**, the board carried from LUL-3325 to LUL-4324, 550 open `Board-integrity:` tickets, and 35% of the CTO's lifetime shipped output spent closing them. The per-subject cooldown cannot stop this, because each generated ticket *is* a genuinely new subject. Two fixes shipped: `findBlockedNoBlockers` now excludes any issue whose title starts with `WAKE_MARKER_PREFIX` (filing a second detector ticket about a stranded detector ticket does not unstick the first), and a hard `MAX_ALARM_F_FILES_PER_SWEEP = 10` backstop — a sweep wanting to file more is reporting one systemic fault, not that many independent ones, so it stops and logs. The general rule: **any alarm that files a ticket which can itself trip that same alarm is a feedback loop, and needs both a structural exclusion and a per-sweep cap.**
...
**There is no backup of this database, and that is the single largest unmitigated risk on the box.** As of 2026-09-19 the data directory is **1.3 GB**, holds every ticket, agent config, heartbeat and run record the studio has ever produced, and nothing dumps it: no cron line, no systemd timer, no `.dump` file anywhere on the host. The `*backup*` directories under `~` are all config and Markdown snapshots (charter files, `AGENTS.md`, instruction diets), not the database. The process is a **child of `paperclip.service`**, so it dies with the server and a `systemctl --user stop paperclip` is also a database stop. Disk is not the constraint — 107 GB free on `/`, 423 GB on `/mnt/hdd` — so the fix is a daily `pg_dump -Fc` to `/mnt/hdd` on a timer with a retention sweep.
...
16 live agents under company `5392c9fe-5b2a-43ee-974f-87a9da51150b`, all `adapterType: claude_local`, in an org chart **four levels deep** (CEO → CTO → Founding Engineer → Game Engineer); every other agent hangs directly off the CEO or the CTO. `orgChainHealth` reads `healthy` for all 16. The sixteenth is **SEO Specialist** (`seo-specialist`), created by the founder on 2026-09-18 — note that `agent create` is refused with a 409 ("Direct agent creation requires board approval") and the working path is `agent hire` followed by `agent approve`. The founder — not the CEO — decides who is paused and which model each agent runs; that rule is written into the CEO's own `AGENTS.md` ("Founder-controlled roster and models — directive 2026-09-07"), after the CEO unpaused three agents and upgraded two models against the founder's intent and was reverted.
...
flowchart TD
  ...
  CEO --> SUM["Summarizer ⏸"]
  CEO --> SEO["SEO Specialist<br/>haiku-4-5"]
...
| SEO Specialist (`seo-specialist`) | CEO | haiku-4-5 | 60 min | 3000 | 30 / 1800 s | error (quota) | Organic discovery for the `lullwood` keyword; **and** board routing of orphaned tickets to the right owner. Does not implement code. |
...
- **The quota → recovery → phantom-block chain is the fleet's most common systemic failure, and it is still live.** When an agent exhausts its Claude quota, `@paperclipai/server/dist/services/recovery/service.js` classifies `provider_quota` as a **transient infra** error (it sits in `TRANSIENT_INFRA_CONTINUATION_ERROR_CODES` alongside `adapter_failed`, `codex_transient_upstream`, `claude_transient_upstream` and `timeout`). Transient means retry, and the default backoff is `PROVIDER_QUOTA_RECOVERY_DEFAULT_BACKOFF_MS = 60 * 60 * 1000` — **one hour**. Real quota windows are 3 h, 24 h, or a **week**. So the retry lands while the agent is still suppressed, fails again, and recovery escalates the ticket to `blocked` with `unresolvedBlockerCount: 0` — a state invisible to every sweep, because nothing is waiting on it and nothing will wake it. The board silently fills with unreachable work: **82 of 123 non-terminal tickets** were in exactly this state on the morning of 2026-09-19, and a single quota-window close has been observed taking `blocked` from 5 to 55.
  The service does try to parse a real reset time out of the error string, but the regex only matches the *daily* shape. Verified against the live strings: `try again at 3pm (Asia/Jerusalem)` and `resets 3:30am (Asia/Jerusalem)` match; **`resets Sep 22, 3pm (Asia/Jerusalem)` — the weekly format, and the one firing across the whole fleet right now — does not**, because the pattern wants a digit immediately after `resets` and the month name stops it. A corrected pattern adds an optional `(?:([A-Z][a-z]{2})\.?\s+(\d{1,2}),?\s+)?` group before the hour. The subtler half matters more than the regex: when a **date** is present the reset is *days* away, so the backoff must be computed to that datetime in the named timezone, not collapsed to an hour-of-day, and a parsed-but-dateless match must fail loudly rather than silently falling back to +1 h. LUL-3251 tracks the fix.
  **This code lives in `node_modules`, not in this repo**, so it cannot be fixed here. `paperclipai` is installed globally at `~/.local/lib/node_modules/paperclipai`, which also means `patch-package` from this repo will not reach it — the repo does not depend on the package. The real options are upstream, or a founder-authorised local patch to the global install. The founder's standing rule is **do not write into `node_modules`**; treat that as unchanged unless they say otherwise on LUL-3251.
  Remediation when it has already happened: move the affected tickets back to `todo` (that is the LUL-3018 rule — never silently flip status without a comment naming the defect). They may flip straight back while the window is closed; that is the same defect, not a new one, and since PR #771 the detector no longer amplifies it.
- **`issue list` caps at exactly 500 rows per query, and it does not tell you it truncated.** Any count taken from `paperclipai issue list --status X` is a **floor, never a total**. This produced three separate wrong numbers in two days on 2026-09-18/19: 93 phantom-blocked tickets reported against a real ~987, 60 detector tickets against ~1055, and a per-agent throughput table that understated shipped counts by factors of 4 to 8 and got two of three noise percentages wrong by more than 20 points. It also explains the older "done count going backwards" puzzle (462 → 422): older `done` rows were being pushed out of the 500-row window by newer ones. To get a real count, either partition the query so no single call can reach 500 — `--status` **and** `--assignee-agent-id` together is usually enough — or read `issues` straight from Postgres with the snippet in §11. Never compare two uncapped-looking totals and infer a trend.
