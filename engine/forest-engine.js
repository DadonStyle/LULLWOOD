...
// LUL-4960: M5 Cold Walk. coldWalkOptIn is the persisted Settings choice (applied at the
// next enter()/restart()); coldWalkBroken is the per-run "already sprinted" fact, reset in
// enter() below.
...
function finishPickup(){
  ...
  const secondaryBonus = secondaryWon
    ...
    : 0;
  // LUL-4960: M5 Cold Walk -- win-only bonus, forfeited (0) the instant the player ever
  // sprints before this pickup() is accepted (coldWalkBroken, set in the movement block).
...
function stepFrame(dt, t, skipRender){
  ...
  if(playing && !hidden){
    ...
    // LUL-4960: M5 Cold Walk -- a sustained-history check ("has the player ever sprinted
    // since run start"), distinct from the per-frame speed/scent-radius consumers of
    // `running` below (SPEC Q7). `!pickingUp` is always true here (this whole block is
    // gated on `playing`, which already excludes pickingUp) but coldWalkJustBroke() stays
    // pure and takes it explicitly per the SPEC's own contract.
