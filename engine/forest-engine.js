...
function hasLOS(x0,z0,x1,z1){
  ...
  for(let i=0; i<=steps; i++){
    ...
    for(const c of arr){ const co=Math.cos(-c.ry),si=Math.sin(-c.ry),dx0=x0-c.x,dz0=z0-c.z,dx1=x1-c.x,dz1=z1-c.z; if(segRayVsAABB(dx0*co-dz0*si,dx0*si+dz0*co, dx1*co-dz1*si,dx1*si+dz1*co, 0,0,c.hx,c.hz)) return false; }
  }
  return true;
}
