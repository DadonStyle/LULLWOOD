    ...
    // spot "alert": brief rear-up + freeze the instant it locks on
      ...
      if(shouldRevertInvestigateToChase(p.inv, hidden)){ p.state='chase'; }
        ...
        if(sniffOutcome.done){
          ...
          else { p.state='roam'; p.spotted=false; logChronicle('predator_gave_up', { kind: p.kind }); }
        }
      } else if(p.inv === 'back'){
      ...
      if(canSee(p, dist)){
        ...
        if(holdOutcome.done){
          ...
          else { p.state='roam'; p.spotted=false; p.inv=''; logChronicle('predator_gave_up', { kind: p.kind }); }
        }
      } else {
