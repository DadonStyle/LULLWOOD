...
interface PersistedSettings {
  ...
  captionsOn: boolean;
  // LUL-4960: M5 Cold Walk opt-in -- defaults false (never pre-selected), applied at the
  // next enter()/restart() same as setDifficulty (see the SPEC's Design call).
...
export default function SettingsPanel({
  ...
  return (
    <div id="settingsPanel" role="dialog" aria-label="Settings">
      ...
      {/* LUL-4960: M5 Cold Walk -- opt-in lives here (persisted, applied at the next
          enter()/restart()), not the one-time #gate screen -- see the SPEC's Design call
          for why #gate can't express an "across many runs" choice. */}
      <fieldset>
