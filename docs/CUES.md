...
| Stone Marker mist-charm | Beacon glow `addBeaconGlow(g, 'stoneMarker', 5.9)` `:1248` | **MISSING** — neither `buyVeilCharm()` (`:4498-4505`) nor the `reserveFired` block (`:5060-5066`) makes any audio call. Both carry an unresolved comment naming `missionCompleteSting()` (`:4605-4618`) as the pattern to mirror. | Offer prompt `:5421` `'Press  E  for a mist-charm  ·  15 embers'` (names the price, not what the charm does); purchase caption `:4502` `'a charm against the mist'`; activation caption `:5063` `'the charm held'` | Tracked already on LUL-2331 (Tier B, assigned, `todo`) — this row does not duplicate a new ticket, see comment posted back on this audit. The offer prompt is the one true `MISSING` explanation cell (purchase/activation captions exist but don't explain the effect either); LUL-2331's plan already covers rewording it. |
...
- **Stone Marker mist-charm** — audio (purchase + activation) MISSING, offer-prompt
  explanation MISSING. Already tracked: LUL-2331 (Tier B, assigned, `todo`, child of
  LUL-2321 Part 1). Not duplicated here.
