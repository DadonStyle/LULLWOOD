...
| Hide-reposition shuffle (LUL-3066) | `rustleFlash = 1; rustleSting()` `:3637` inside `shuffleHide()` (`:3598-3657`) — same green vignette pulse `rollCoverRustle()` uses, fired unconditionally on every successful shuffle (not just downwind, per the merged SPEC's Deviation 5: `spotFlash` is reserved for a real detection) | `leafRustle(movingAgainstWind)` `:3625` — muffled (2-burst) upwind, fuller (3-burst) downwind, reusing the existing enter/exit-hide foley rather than a new sound | Transient caption toast `:3651/:3653` — `'Shifted position'` / `'Shifted position — noisy'`, every use; first use instead gets `'Press R to shift position within cover — silent upwind, noisy downwind.'` (one caption slot per call, not two — see the PR's declared deviation on why the merged SPEC's separate HINT_PRIORITY entry isn't wired: `baseHintEligible` requires `!hidden`, and this feature's precondition is `hidden`) | Full triple present. `KeyR` + touch Shuffle button (`components/MobileControls.tsx`), only while `hidden` and off `shuffleCooldownAccum`. |
