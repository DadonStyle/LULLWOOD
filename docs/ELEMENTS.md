...
- Move (WASD/arrows), walk or run (`Shift`, hold by default; toggle if the
  ...
  `triggerTouchToggleRun()` (L3392-3396, gated on the same
...
- Jump at any time while playing, not gated on being chased — `beginJump()`,
  ...
  `triggerTouchJump()` (L3369-3375, same guards as the desktop `Space`
...
- Pause the run (`Escape`, desktop-only key) or resume it — touch has no
  ...
  (L3381-3385, `MobileControls.tsx`'s `touchPause` button) toggles both
...
- Dim the personal follow-light (hold `KeyF`, or hold touch's `touchVeil`
  button via `setTouchVeil()` L3355 — `veilHeld` reads `keys['KeyF'] ||
  touchVeil` at L2976, mirrored the same way in `qaPlayerState()`'s return
