...
- Dim the personal follow-light (hold `KeyF`, or hold touch's `touchVeil`
  button via `setTouchVeil()` L7522 — `veilHeld` reads `keys['KeyF'] ||
  touchVeil` at L6707, mirrored the same way in `qaPlayerState()`'s return  object, so the two inputs are equivalent, not independent) —
...
- `win`/`loss` telemetry events (LUL-1450): `difficulty: Difficulty` field added to
  both `track()` call sites, in `finishPickup()` (L6057-6117, the win path since
  `LUL-2281`) and `triggerDeath()` (L6298-6339). The `difficulty` module-level
...
- `loss` telemetry event (LUL-2461): `distance_from_home_m` field added --
  ...
  (L6298-6339) fires, computed and stored in `deathDistanceFromHomeM` (module-level,
...
- `livePileEmbers` (LUL-1315): live, unbanked depth+survival total for the
  ...
  reset to 0 on `enter()` (L4063) and recomputed every frame (`stepFrame()`,
...
**What it can do**
- Gate the player's sprint speed (`stepFrame()` at L6657-7458, LUL-2071's extracted per-frame body): `maxSpd = (running ? walk*sprintSpeedMul(staminaCharge) : walk) * ...`, so the player still moves at walk pace when running with zero stamina, but gains speed as stamina refills.
...
`windAssistStartCue()` (L6238) and `windAssistEndCue()` (L6247), edge-triggers on the combined
