...
- Dim the personal follow-light (hold `KeyF`, or hold touch's `touchVeil`
  button via `setTouchVeil()` L7832 — `veilHeld` reads `keys['KeyF'] ||
  touchVeil` at L6883, mirrored the same way in `qaPlayerState()`'s return  object, so the two inputs are equivalent, not independent) —
...
- `win`/`loss` telemetry events (LUL-1450): `difficulty: Difficulty` field added to
  both `track()` call sites, in `finishPickup()` (L6115-6176, the win path since
  `LUL-2281`) and `triggerDeath()` (L6487-6528). The `difficulty` module-level
...
- `loss` telemetry event (LUL-2461): `distance_from_home_m` field added --
  ...
  (L6487-6528) fires, computed and stored in `deathDistanceFromHomeM` (module-level,
  set at L6495) rather than recomputed later, since `player.x/z` can move on
...
- `livePileEmbers` (LUL-1315): live, unbanked depth+survival total for the
  run in progress — `hudState` field (`engine/forest-engine.js` L3897),
  reset to 0 on `enter()` (L3988) and recomputed every frame (`stepFrame()`,
  called each `tick()` -- LUL-2071 extracted the per-frame body out of `tick()`
  so a QA test clock can call it directly) while the run
  is neither won nor dead (L7095: `computeDepth(maxDistFromHome) +
...
- **Implemented (LUL-1259, widened LUL-3010, LUL-4958, LUL-4900).** `MISSION_POOL`
  ...
  `engine/tuning.js:68`) — and `flush` (LUL-5116) — no world target, completes when a
  player-thrown stone flushes one specific roost of the 5 fixed `ROOSTS`
  (`engine/tuning.js:75`), named at mission-draw time. Per-run state
  (`mission: MissionState | null`) lives alongside `baby` at `engine/forest-engine.js:885`,
  drawn once per `generateMap()` call, after every other rng() consumer, so it never shifts
  the stream any existing seed/replay depends on.

**Six variants (LUL-3010, LUL-4958, LUL-4900, LUL-5116)**
...
- `flush` (LUL-5116) — no world target (`spatial: false`, `lib/game/mission.ts`), same
  ...
  `placeCave()` (the new last consumer in the stream, per LUL-1904). Completes on the
  player-thrown roost-flush branch of `throwThrowable()` (`canCompleteFlush()`,
  `lib/game/mission.ts`) — deliberately NOT on the pre-existing ambient chase-proximity
  `updateRoosts()` path, which never calls `canCompleteFlush()` at all, so a different
  (unmarked) roost being flushed by a wandering predator cannot complete this mission.
...
- Add a completion bonus to the win payout only, keyed by kind via `MISSION_REWARDS`
  ...
  MISSION_STONE_MARKER_REWARD = 7`, `radioMast: MISSION_RADIO_MAST_REWARD = 3`, `flush:
  MISSION_FLUSH_REWARD = 9` (placeholder)), passed as
...
speed bonus applied to `spd` inside `stepFrame()` (L7039), and a `NOISE_RADIUS_RUN_WIND` (16.8, `lib/game/noise.ts`)
footstep-radius reduction applied to `noiseRadius` (L7055), replacing the plain sprint radius
...
(`engine/forest-engine.js` L7650 for the eligibility case), positioned below the danger hints
...
`windAssistStartCue()` (L6413) and `windAssistEndCue()` (L6422), edge-triggers on the combined
