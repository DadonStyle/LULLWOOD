...
- Dim the personal follow-light (hold `KeyF`, or hold touch's `touchVeil`
  button via `setTouchVeil()` L6779 — `veilHeld` reads `keys['KeyF'] ||
  touchVeil` at L5980, mirrored the same way in `qaPlayerState()`'s return  object, so the two inputs are equivalent, not independent) —
...
- `win`/`loss` telemetry events (LUL-1450): `difficulty: Difficulty` field added to
  both `track()` call sites, now in `finishPickup()` (L5383, the live win path as
  of `LUL-2281` -- `arriveHome()`'s L5534 copy is unreachable, kept per Decision 2)
  and `triggerDeath()` (L5588). The `difficulty` module-level variable is in scope
...
- `loss` telemetry event (LUL-2461): `distance_from_home_m` field added --
  ...
  (L5588) fires, computed and stored in `deathDistanceFromHomeM` (module-level,
  set at L5595) rather than recomputed later, since `player.x/z` can move on
...
- `livePileEmbers` (LUL-1315): live, unbanked depth+survival total for the
  ...
  reset to 0 on `enter()` (L3770) and recomputed every frame (`stepFrame()`,
