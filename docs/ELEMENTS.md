...
- Move (WASD/arrows), walk or run (`Shift`, hold by default; toggle if the
  ...
  `triggerTouchToggleRun()` (L3175-3179, gated on the same
...
- Jump at any time while playing, not gated on being chased — `beginJump()`,
  ...
  `triggerTouchJump()` (L3152-3158, same guards as the desktop `Space`
...
- Pause the run (`Escape`, desktop-only key) or resume it — touch has no
  ...
  (L3164-3168, `MobileControls.tsx`'s `touchPause` button) toggles both
...
- Dim the personal follow-light (hold `KeyF`, or hold touch's `touchVeil`
  button via `setTouchVeil()` L3138 — `veilHeld` reads `keys['KeyF'] ||
  touchVeil` at L2807, mirrored the same way in `qaPlayerState()`'s return
