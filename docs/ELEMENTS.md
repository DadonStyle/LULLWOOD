...
- Dim the personal follow-light (hold `KeyF`, or hold touch's `touchVeil`
  button via `setTouchVeil()` L7463 — `veilHeld` reads `keys['KeyF'] ||
  touchVeil` at L6656, mirrored the same way in `qaPlayerState()`'s return  object, so the two inputs are equivalent, not independent) —
...
- `win`/`loss` telemetry events (LUL-1450): `difficulty: Difficulty` field added to
  both `track()` call sites, in `finishPickup()` (L6018-6078, the win path since
  `LUL-2281`) and `triggerDeath()` (L6247-6288). The `difficulty` module-level
...
- `loss` telemetry event (LUL-2461): `distance_from_home_m` field added --
  ...
  (L6247-6288) fires, computed and stored in `deathDistanceFromHomeM` (module-level,
...
**What it can do**
- Gate the player's sprint speed (`stepFrame()` at L6606-7399, LUL-2071's extracted per-frame body): `maxSpd = (running ? walk*sprintSpeedMul(staminaCharge) : walk) * ...`, so the player still moves at walk pace when running with zero stamina, but gains speed as stamina refills.
...
`windAssistStartCue()` (L6199) and `windAssistEndCue()` (L6208), edge-triggers on the combined
