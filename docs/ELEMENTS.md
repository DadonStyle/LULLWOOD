...
- Dim the personal follow-light (hold `KeyF`, or hold touch's `touchVeil`
  button via `setTouchVeil()` L7699 — `veilHeld` reads `keys['KeyF'] ||
  touchVeil` at L6756, mirrored the same way in `qaPlayerState()`'s return  object, so the two inputs are equivalent, not independent) —
...
- `win`/`loss` telemetry events (LUL-1450): `difficulty: Difficulty` field added to
  both `track()` call sites, in `finishPickup()` (L6073, the win path since
  `LUL-2281`) and `triggerDeath()` (L6360-6401). The `difficulty` module-level
...
- `loss` telemetry event (LUL-2461): `distance_from_home_m` field added --
  ...
  (L6360-6401) fires, computed and stored in `deathDistanceFromHomeM` (module-level,
...
- `livePileEmbers` (LUL-1315): live, unbanked depth+survival total for the
  ...
  reset to 0 on `enter()` (L3917) and recomputed every frame (`stepFrame()`,
...
`windAssistStartCue()` (L6286) and `windAssistEndCue()` (L6295), edge-triggers on the combined
