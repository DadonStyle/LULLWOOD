...
- Move (WASD/arrows), walk or run (`Shift`, hold by default; toggle if the
  ...
  `triggerTouchToggleRun()` (L3233-3237, gated on the same  `runMode==='toggle'` check; `MobileControls.tsx`'s `touchToggleRun` button
...
- Jump at any time while playing, not gated on being chased — `beginJump()`,
  ...
  `triggerTouchJump()` (L3210-3216, same guards as the desktop `Space`  keydown handler, minus the `e.repeat` check since a tap is already
...
- Pause the run (`Escape`, desktop-only key) or resume it — touch has no
  ...
  (L3222-3226, `MobileControls.tsx`'s `touchPause` button) toggles both  directions instead of only pausing.
...
- Dim the personal follow-light (hold `KeyF`, or hold touch's `touchVeil`
  button via `setTouchVeil()` L3196 — `veilHeld` reads `keys['KeyF'] ||
  touchVeil` at L2856, mirrored the same way in `qaPlayerState()`'s return
...
| **WO** Wolf | | | ·⁹ | **U**¹⁰ | **U**¹⁰ | C(trunk)+LOS³ | LOS only¹¹ | LOS only¹¹ | LOS only¹¹ | STAND | –¹² | – | – | – | TRIG⁶ |
| **BE** Bear | | | | –¹³ | **U**¹⁰ | C(trunk)+LOS³ | LOS only¹¹ | LOS only¹¹ | LOS only¹¹ | STAND | –¹² | – | – | – | TRIG⁶ |
| **LI** Lion | | | | | –¹³ | C(trunk)+LOS³ | LOS only¹¹ | LOS only¹¹ | LOS only¹¹ | STAND | –¹² | – | – | – | TRIG⁶ |
