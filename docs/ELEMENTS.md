...
- Move (WASD/arrows), walk or run (`Shift`, hold by default; toggle if the
  ...
  `triggerTouchToggleRun()` (L3158-3162, gated on the same
...
- Dim the personal follow-light (hold `KeyF`, or hold touch's `touchVeil`
  button via `setTouchVeil()` L3121 — `veilHeld` reads `keys['KeyF'] ||
  touchVeil` at L2790, mirrored the same way in `qaPlayerState()`'s return
