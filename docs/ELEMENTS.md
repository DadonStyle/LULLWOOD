...
- Dim the personal follow-light (hold `KeyF`, or hold touch's `touchVeil`
  button via `setTouchVeil()` L7095 — `veilHeld` reads `keys['KeyF'] ||
  touchVeil` at L6290, mirrored the same way in `qaPlayerState()`'s return  object, so the two inputs are equivalent, not independent) —
...
- `win`/`loss` telemetry events (LUL-1450): `difficulty: Difficulty` field added to
  both `track()` call sites, now in `finishPickup()` (L5684-5744, the live win path as
  of `LUL-2281` -- `arriveHome()`'s L5835-5888 copy is unreachable, kept per Decision 2)
  and `triggerDeath()` (L5889-5932). The `difficulty` module-level variable is in scope
...
- `loss` telemetry event (LUL-2461): `distance_from_home_m` field added --
  ...
  (L5889-5932) fires, computed and stored in `deathDistanceFromHomeM` (module-level,
...
**What it can do**
- Gate the player's sprint speed (`stepFrame()` at L6243-7031, LUL-2071's extracted per-frame body): `maxSpd = (running ? walk*sprintSpeedMul(staminaCharge) : walk) * ...`, so the player still moves at walk pace when running with zero stamina, but gains speed as stamina refills.
