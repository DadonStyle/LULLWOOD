...
- Move (WASD/arrows), walk or run (`Shift`, hold by default; toggle if the
  ...
  `triggerTouchToggleRun()` (L3333-3337, gated on the same
...
- Jump at any time while playing, not gated on being chased — `beginJump()`,
  ...
  `triggerTouchJump()` (L3310-3316, same guards as the desktop `Space`
...
- Pause the run (`Escape`, desktop-only key) or resume it — touch has no
  ...
  (L3322-3326, `MobileControls.tsx`'s `touchPause` button) toggles both
...
- Dim the personal follow-light (hold `KeyF`, or hold touch's `touchVeil`
  button via `setTouchVeil()` L3296 — `veilHeld` reads `keys['KeyF'] ||
  touchVeil` at L2926, mirrored the same way in `qaPlayerState()`'s return
...
- Stuck detection: if a predator's actual movement falls under 35% of its
  ...
  trail points then picks a fresh random waypoint (`p.stuckT`, L1465-1470).
