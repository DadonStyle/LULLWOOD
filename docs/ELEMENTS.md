...
Source of truth: `engine/forest-engine.js` on `main` at `fc2b51f` (2026-08-18),
...
**What it can do**
- Move (WASD/arrows), walk or run (`Shift`, or toggle-run per `runMode`),
  look (mouse via Pointer Lock, or drag-fallback, or touch stick on mobile) —
  `applyLook()` L1251, movement block in `tick()` L2273-2304.
- Jump at any time while playing, not gated on being chased — `beginJump()`
  L1208-1211, `JUMP_DURATION`/`JUMP_HEIGHT` in `lib/game/jump.ts`. The same
...
- Enter a `hidden` stance (`KeyH` / touch Hide) — but **only** while standing
  ...
  (`HIDE_KINDS`, L270-271; `findHideSpot()` L865-878). Hiding lowers eye
  ...
  next to a predator still gets you caught — `effectiveDetect()` L882-885).
- Dim the personal follow-light (hold `KeyF`) — `LIGHT_NORMAL`/`LIGHT_DIMMED`
  L164-165, applied in `tick()` L2262-2271; paired with a screen-edge
  vignette cue (`applyVignette()` L183-187), not a detection multiplier yet
  (LUL-40's own comment, L157-163: dormant, reserved for `effectiveDetect()`).
- Pick up the child (`KeyE` / touch Interact) within 3.6 units, once
  (`canPickup`, `pickup()` L2008-2018).
- Carry the child home; walking speed is multiplied by `CONFIG.carryPaceMul`
  (0.72) while carrying (`tick()` L2276).
- Leave a scent trail while moving (not while hidden or standing still) —
  `depositScent()` L763-766, deposited every `SCENT_DEPOSIT_INTERVAL` (0.3s).
- Make audible footstep noise while moving — `NOISE_RADIUS_WALK`/`_RUN`
  (14/24 units), `checkNoise()` L808-811.
- Dodge a telegraphed predator charge by jumping within the charge window —
  `stepCharge()` in `lib/game/charge.ts`.
- Change accessibility/control settings live: run mode (hold/toggle), mouse
  sensitivity, invert-Y, reduced motion, captions, difficulty (LUL-26,
  `setRunMode`/`setSensitivity`/`setInvertY`/`setReducedMotion`/
  `setCaptions`/`setDifficulty`, L2089-2107).
...
- Cannot enter `hidden` anywhere else — standing behind a rock or a
  ...
  incidentally while you keep moving (LUL-212's own framing, L255-269).
...
- Movement collision: `blocked(x,z) = blockedR(x,z,0.6) || coverBlockedR(x,z,0.6) || canopyBlockedR(x,z)`
  (L364) — the **only** consumer of `canopyBlockedR()`/`coverBlockedR()` in
...
- `toggleHidden()` is declared **twice** in the same closure scope (plain
  `function` statements, not `const`): once at L1198-1201 (the original
  LUL-153 version: unconditional `hidden=!hidden` + a `feature_engagement`
  analytics `track()` call), and again at L1409-1413 (the LUL-212 rewrite:
...
- Eye height (`eyeH`) is damped toward `hidden ? 1.05 : CONFIG.eye` (2.2) at
  an ~0.3s time constant (`Math.min(1, dt*8)`, L2249), not snapped — see
...
- Player FOV for "can the player see the charging predator" gating is ~130°
  total (`PLAYER_FOV_COS`, `cos(65°)`, L1662) — independent of the render
  camera's own 70° vertical FOV (`camera` L97); this is a gameplay cone, not
...
- No vertical/ground collision at all: eye height is a formula
  (`eyeH + bob + jumpY`, L2352), never a raycast against the ground mesh.
- Two different downstream checks read player position without going through
  ...
  (`checkScent()` L767-773 and `checkNoise()` L808-811 take no cover/LOS
  argument at all).
...
- Sit at a fixed point drawn once per map (`baby.x/z`, `generateMap()`
  L423-427), glowing and idly bobbing, marked by ambient "wisp" particles
  (`placeBabyWisps()` L539-543) so it's spottable through fog.
- Be picked up once (`baby.taken`, `pickup()` L2008), triggering a scripted
  10s pickup cinematic (`tick()`'s `pickingUp` branch, L2306-2335) that ends
  in a sky-burst (`fireBoom()` L571-579).
- Ride along at the player's position while carried, small and glowing
  (`carrying` branch, `tick()` L2336-2345), until the player crosses
  `CONFIG.home.r` (3.6u) of the home landmark, which wins the run
  (`arriveHome()` L2032-2039).
- Have its idle/carry glow intensity scaled by the active difficulty preset's
  `glowMul` (LUL-26, `DIFFICULTY_PRESETS`, L624-628).
...
- Cannot be found or interacted with by anything except the player — no
  ...
  of it — `Math.hypot(x-baby.x,z-baby.z)<26` in `placePredators()` L708).
...
**Behaviours & logic**
- Placement: `generateMap()` L423-427 — polar draw, `d = half*(0.5+rng()*0.3)`
...
- Home is a **fixed reuse of the spawn point** (`CONFIG.home = {x:0,z:0,...}`,
  L75, comment: "reuses the spawn point, no new rng draw" — LUL-38), not a
  second procedurally-placed landmark.

**Collision & physics profile**
- No collider. Visual scale/position only (`babyGroup`, a `THREE.Group` of
  two spheres + a halo + a point light, L518-531). Placement-time-only
  clearance from the lake (`inLake`) and from trees/cover (`inBaby()`,
  L516, used symmetrically by *their* placement loops, not the child's own).

---

### Wolf / Bear / Lion (predators)

Three species sharing one state machine (`updatePredators()`, L941-1169) and
one geometry builder (`makePredator()`, L630-688), differentiated by the
`PSPEC` table (L603-611):
...
- Roam via random waypoints when nothing has noticed the player
  (`state==='roam'`, L995-1004).
- Detect the player through three independent channels: **sight**
  (`canSee()`, LOS raycast + shrinking-with-stillness range, L886-889),
  **scent** (`checkScent()`, radius+wind, no LOS check at all, L767-773),
  and **noise** (`checkNoise()`, pure distance + per-second chance while the
  player moves, L808-811). Any one channel alone triggers a chase.
...
- Force-hunt: if nothing has been within 20 units of the player for 30s, the
  ...
  break) — `tick()` L2379-2381.
- **Wolf only**: coordinate as a pack. The instant one wolf chases, the other
  two (if any, per difficulty preset) path to flanking points ±60° off the
  player's last movement heading (`updateWolfPack()`, L905-932).
- **Wolf and lion only**: telegraph-and-charge at 7-16 units range
  ...
  threat" (L1017-1018 comment) — contrast with wolf/lion is the design
...
- Get parked off-map (`p.inert`, `x=z=-9999`) under lower difficulty presets
  that reduce `activePerSpecies` (LUL-26, `placePredators()` L696-719).
...
- Cannot physically collide with cover props (rock/log/bramble) at all —
  ...
  **Deliberate**, not a gap: the standing comment at `coverBlockedR()`
  (L300-312) says folding this in previously produced a stuck-predator
...
- Cannot be avoided by outrunning them (every species is faster than the
  ...
  speeds exceed it — L615-616).
- Cannot spawn inside the spawn clearing, too close to the child, or inside
  another collider (`placePredators()`'s rejection loop, L707-708) — **but
...
**Behaviours & logic**
- `PSPEC`'s literal `speed` values (8.5/6.8/9.2, L608-610) are **immediately
  overwritten** by `for(const k in PSPEC) PSPEC[k].speed = RUN + CHASE_GAP/PSPEC[k].budget`
  (L615-616) before any predator object is created — the literals in the
...
- Charge state machine (`telegraph`→`charging`→`overshoot`→`caught`/`cleared`)
  ...
  `CHARGE_COOLDOWN`=10s, L939) and the resulting movement.
- Stuck detection: if a predator's actual movement falls under 35% of its
  ...
  trail points then picks a fresh random waypoint (`p.stuckT`, L1104-1112).
...
- Additionally block the **player's own** movement via a wider,
  ...
  L220-223) — player-only, see wiki `game/lul267-canopy-collision-fix` for
...
- 1,300 instances (`CONFIG.trees`), one instanced trunk cylinder + two
  instanced foliage cones (`trunkGeo`/`cone1Geo`/`cone2Geo`, L192-194),
  scale `s = 0.7 + rng()*1.7` drawn per tree (`generateMap()` L439).
...
- LOS: rotated-AABB `{hx:hz: t.cr*1.4}` (L379), tagged trees only.
...
- Render as one of three cover-prop kinds (`DodecahedronGeometry`, L243),
...
- Everything Rock can do, **plus**: is a valid `hidden`-stance location
  ...
  log knock" sound (`hollowLogSound()`, L1380-1401).
- ~40% of cover-prop rolls (`roll < 0.4`, `generateCover()` L388), long/thin
...
- `long = 1.3+rng()*1.1, thin = 0.35+rng()*0.25`, orientation randomized
  between long-on-x / long-on-z (`generateCover()` L388-389).
...
- Additionally gates `findHideSpot()` (proximity search, `HIDE_RADIUS`=2.2u
  beyond the prop's own edge, L865-878).
...
- Everything Log can do (hiding-spot eligible, `HIDE_KINDS.bramble = true`),
  ...
  L1359-1376) — researched against stealth/horror foley convention per the
  LUL-212 handoff (wiki `game/lul212-hiding-spots`).
- ~25% of cover-prop rolls (`roll >= 0.75`, `generateCover()` L391).
...
**What it can do**
- Render as a single 800×800 flat plane (`PlaneGeometry`, L130-132),
...
**Behaviours & logic**
- Single static mesh, created once, never touched again after L130-132.
...
- Keep other elements clear of itself **at placement time only**: trees,
  ...
  `CONFIG.lake.clear` (22 units, `inLake()` L281, used at L385, L427, L438).
- Bias the ambient "twinkle" chime to play brighter/more often when the
  player is near it (`distLake < CONFIG.lake.r*3`, `tick()` L2468-2470).
...
- Does not keep **predators** clear of itself at spawn — `placePredators()`'s
  rejection loop (L707-708) checks spawn-clearing distance, baby distance,
...
**Behaviours & logic**
- `CONFIG.lake = { x:34, z:-28, r:15, clear:22, glow:0x86b8ff }` (L74).
- Ambient wisp particles loop 0.2→4.5 units and reset (`tick()` L2482-2484).
...
- Mark the win destination: a point light + additive ring at
  `CONFIG.home = {x:0, z:0, r:3.6, glow:0xffd9b0}` (L75, L484-489).
- Trigger the win condition when the player, while `carrying`, comes within
  `CONFIG.home.r` of it (`arriveHome()`, `tick()` L2344-2345).
...
- Is not itself protected from tree/cover placement by name — it is
  ...
  spawn clearing (`inSpawn(x,z) = x*x+z*z<40`, L282), which trees and cover
  both already avoid (LUL-38 comment, L75: "reuses the spawn point, no new
...
- Is not drawn on the minimap (`drawMinimapStatic()` renders trees and the
  lake only, L2118-2126 — home has no minimap marker).
...
- Uniformly fade all rendered fragments by camera distance
  (`scene.fog = new THREE.FogExp2(0x0b1220, CONFIG.fog)`, L95,
  `CONFIG.fog = 0.04`, L68).
- Be adjusted live by the player via the settings panel (`setFog()`,
  L2078) — a pure rendering parameter, `scene.fog.density`.
...
- Excluded from a handful of unfogged/always-visible effects on purpose
  ...
  L146-153, L560-567) so those read clearly regardless of density.
...
- Illuminate the area around the player, attached directly to the camera
  ...
  L156) — always exactly coincident with the player, not a separate
  tracked entity.
- Switch between two fixed states, `LIGHT_NORMAL`/`LIGHT_DIMMED`
  (intensity 0.7/0.18, distance 20/8, L164-165), toggled by holding `KeyF`
  (`tick()` L2262-2271), paired with a screen vignette cue.
...
- Does not feed into `effectiveDetect()` yet — LUL-40's own comment (L157-163)
  states this is deliberately dormant, reserved for a future detection-math
  wire-up, "ship the control first."
...
- **React-owned** (`components/Hud.tsx`), driven one-directionally by
  `hudState`/`pushState()`/`emitState()` (L1614-1637): objective text,
  hiding status, win/death screens, charge-dodge prompt, difficulty/
  accessibility controls, captions, the post-run recap (`#runRecap`).

**What it can do**
- Render every piece of state the engine pushes (`pushState()`, only sends
  a patch when a value actually changed, L1630-1636).
- Send **actions back**, never state: `enter`, `restart`, `setPace`,
  `setFog`, `toggleSound`, `regenMap`, touch-control setters, and the
  LUL-26 accessibility/difficulty setters (the full returned API,
  L2582-2584) — these are the *only* way React code can affect the world.
- The minimap specifically reads and draws two other elements' live data:
  ...
  (`drawMinimapStatic()` L2118-2126) — not just player/child/predator state.
...
- Cannot affect physics, collision, or AI directly — even the setters that
  do change world state (`setFog`, `regenMap`, `setDifficulty`) go through
...
- `hudState` is a single flat object; `pushState()` diffs before emitting to
  avoid redundant React re-renders (L1630-1636).
...
⁸ Child's spawn draw rejects `inLake()` positions (`generateMap()` L424-427)
...
`coverBlockedR()` L300-312), not `U`. LOS is still blocked normally.
¹² **Notable.** `placePredators()`'s spawn-rejection loop (L707-708) checks
...
¹⁴ **Notable.** `generateCover()` (L377-396) never checks tree positions —
...
(`generateMap()` L438, `generateCover()` L385) — defined, not undefined.
...
L2118-2126) — a read-only relationship, not physical.
