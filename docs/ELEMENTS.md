...
- Dim the personal follow-light (hold `KeyF`, or hold touch's `touchVeil`
  button via `setTouchVeil()` L7382 — `veilHeld` reads `keys['KeyF'] ||
  touchVeil` at L6501, mirrored the same way in `qaPlayerState()`'s return  object, so the two inputs are equivalent, not independent) —
...
- `win`/`loss` telemetry events (LUL-1450): `difficulty: Difficulty` field added to
  both `track()` call sites, in `finishPickup()` (L5865-5925, the win path since
  `LUL-2281`) and `triggerDeath()` (L6106-6147). The `difficulty` module-level
...
- `loss` telemetry event (LUL-2461): `distance_from_home_m` field added --
  ...
  (L6106-6147) fires, computed and stored in `deathDistanceFromHomeM` (module-level,
  set at L6114) rather than recomputed later, since `player.x/z` can move on
...
`windAssistStartCue()` (L6046) and `windAssistEndCue()` (L6055), edge-triggers on the combined
