...
- Dim the personal follow-light (hold `KeyF`, or hold touch's `touchVeil`
  button via `setTouchVeil()` L7039 — `veilHeld` reads `keys['KeyF'] ||
  touchVeil` at L6239, mirrored the same way in `qaPlayerState()`'s return  object, so the two inputs are equivalent, not independent) —
...
- `win`/`loss` telemetry events (LUL-1450): `difficulty: Difficulty` field added to
  both `track()` call sites, in `finishPickup()` (L5628-5688, the win path since
  `LUL-2281`) and `triggerDeath()` (L5845-5886). The `difficulty` module-level
...
- `loss` telemetry event (LUL-2461): `distance_from_home_m` field added --
  ...
  (L5845-5886) fires, computed and stored in `deathDistanceFromHomeM` (module-level,
...
- `livePileEmbers` (LUL-1315): live, unbanked depth+survival total for the
  ...
  reset to 0 on `enter()` (L3746) and recomputed every frame (`stepFrame()`,
  called each `tick()` -- LUL-2071 extracted the per-frame body out of `tick()`
  so a QA test clock can call it directly) while the run
  is neither won nor dead (L6381: `computeDepth(maxDistFromHome) +
...
**What it can do**
- Gate the player's sprint speed (`stepFrame()` at L6302, LUL-2071's extracted per-frame body): `maxSpd = (running ? walk*sprintSpeedMul(staminaCharge) : walk) * ...`, so the player still moves at walk pace when running with zero stamina, but gains speed as stamina refills.
...
`windAssistStartCue()` (L5809) and `windAssistEndCue()` (L5818), edge-triggers on the combined
