...
- Dim the personal follow-light (hold `KeyF`, or hold touch's `touchVeil`
  ...
  touchVeil` at L6321, mirrored the same way in `qaPlayerState()`'s return  object, so the two inputs are equivalent, not independent) —
...
- `win`/`loss` telemetry events (LUL-1450): `difficulty: Difficulty` field added to
  both `track()` call sites, in `finishPickup()` (L5698-5758, the win path since
  `LUL-2281`) and `triggerDeath()` (L5927-5968). The `difficulty` module-level
...
- `loss` telemetry event (LUL-2461): `distance_from_home_m` field added --
  ...
  (L5927-5968) fires, computed and stored in `deathDistanceFromHomeM` (module-level,
...
`windAssistStartCue()` (L5879) and `windAssistEndCue()` (L5888), edge-triggers on the combined
