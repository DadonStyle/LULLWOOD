...
- Dim the personal follow-light (hold `KeyF`, or hold touch's `touchVeil`
  button via `setTouchVeil()` L7250 — `veilHeld` reads `keys['KeyF'] ||
...
- `win`/`loss` telemetry events (LUL-1450): `difficulty: Difficulty` field added to
  both `track()` call sites, in `finishPickup()` (L5760-5820, the win path since
  `LUL-2281`) and `triggerDeath()` (L5989-6030). The `difficulty` module-level
...
- `loss` telemetry event (LUL-2461): `distance_from_home_m` field added --
  ...
  (L5989-6030) fires, computed and stored in `deathDistanceFromHomeM` (module-level,
...
`windAssistStartCue()` (L5941) and `windAssistEndCue()` (L5950), edge-triggers on the combined
