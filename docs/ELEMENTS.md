...
- Dim the personal follow-light (hold `KeyF`, or hold touch's `touchVeil`
  button via `setTouchVeil()` L6110 — `veilHeld` reads `keys['KeyF'] ||
  touchVeil` at L5373, mirrored the same way in `qaPlayerState()`'s return  object, so the two inputs are equivalent, not independent) —
...
- `win`/`loss` telemetry events (LUL-1450): `difficulty: Difficulty` field added to
  ...
  and `triggerDeath()` (L5010). The `difficulty` module-level variable is in scope
...
- `livePileEmbers` (LUL-1315): live, unbanked depth+survival total for the
  ...
  reset to 0 on `enter()` (L3426) and recomputed every frame (`stepFrame()`,
