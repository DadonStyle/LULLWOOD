...
- Move (WASD/arrows), walk or run (`Shift`, hold by default; toggle if the
  ...
  `triggerTouchToggleRun()` (L3641-3645, gated on the same  `runMode==='toggle'` check; `MobileControls.tsx`'s `touchToggleRun` button
...
- Jump at any time while playing, not gated on being chased — `beginJump()`,
  ...
  `triggerTouchJump()` (L3618-3624, same guards as the desktop `Space`  keydown handler, minus the `e.repeat` check since a tap is already
...
- Pause the run (`Escape`, desktop-only key) or resume it — touch has no
  ...
  (L3630-3634, `MobileControls.tsx`'s `touchPause` button) toggles both  directions instead of only pausing.
...
- Dim the personal follow-light (hold `KeyF`, or hold touch's `touchVeil`
  button via `setTouchVeil()` L3603 — `veilHeld` reads `keys['KeyF'] ||
  touchVeil` at L3183, mirrored the same way in `qaPlayerState()`'s return  object, so the two inputs are equivalent, not independent) —
...
- `embersBalance`: player's persisted currency balance (runs completed,
  ...
  and synced to `hudState` via `setEmbers()` (L3019-3023 in  `engine/forest-engine.js`). Earnable via `computeWinPayout()` /
...
- Reset to full on each new run: `staminaCharge = 1` on `restart()` (L2960, alongside `staminaLowCuePlayed`).
**What it CANNOT do**
...
- No verbs of its own — completion rides the existing interact action (`KeyE`
  (`engine/forest-engine.js:1689`) / `triggerTouchInteract()` (`:3608`), the same key/button
