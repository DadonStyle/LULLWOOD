...
- Dim the personal follow-light (hold `KeyF`, or hold touch's `touchVeil`
  button via `setTouchVeil()` L7615 — `veilHeld` reads `keys['KeyF'] ||
  touchVeil` at L6711, mirrored the same way in `qaPlayerState()`'s return  object, so the two inputs are equivalent, not independent) —
...
- `win`/`loss` telemetry events (LUL-1450): `difficulty: Difficulty` field added to
  both `track()` call sites, in `finishPickup()` (L6003-6063, the win path since
  `LUL-2281`) and `triggerDeath()` (L6316-6357). The `difficulty` module-level
...
- `loss` telemetry event (LUL-2461): `distance_from_home_m` field added --
  ...
  (L6316-6357) fires, computed and stored in `deathDistanceFromHomeM` (module-level,
...
`windAssistStartCue()` (L6256) and `windAssistEndCue()` (L6265), edge-triggers on the combined
