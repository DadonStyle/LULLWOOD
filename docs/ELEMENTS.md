...
- Dim the personal follow-light (hold `KeyF`, or hold touch's `touchVeil`
  button via `setTouchVeil()` L4109 — `veilHeld` reads `keys['KeyF'] ||
  touchVeil` at L3676, mirrored the same way in `qaPlayerState()`'s return  object, so the two inputs are equivalent, not independent) —
...
- `win`/`loss` telemetry events (LUL-1450): `difficulty: Difficulty` field added to
  both `track()` call sites in `arriveHome()` (L3369) and `triggerDeath()` (L3399).
