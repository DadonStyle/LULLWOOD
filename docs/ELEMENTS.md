...
- Move (WASD/arrows), walk or run (`Shift`, hold by default; toggle if the
  ...
  `triggerTouchToggleRun()` (L3448-3452, gated on the same
...
- Jump at any time while playing, not gated on being chased — `beginJump()`,
  ...
  `triggerTouchJump()` (L3425-3431, same guards as the desktop `Space`
...
- Pause the run (`Escape`, desktop-only key) or resume it — touch has no
  ...
  (L3437-3441, `MobileControls.tsx`'s `touchPause` button) toggles both
...
- Dim the personal follow-light (hold `KeyF`, or hold touch's `touchVeil`
  button via `setTouchVeil()` L3411 — `veilHeld` reads `keys['KeyF'] ||
  touchVeil` at L3031, mirrored the same way in `qaPlayerState()`'s return
...
- `embersBalance`: player's persisted currency balance (runs completed,
  ...
  and synced to `hudState` via `setEmbers()` (L2867-2871 in
