...
- Dim the personal follow-light (hold `KeyF`, or hold touch's `touchVeil`
  button via `setTouchVeil()` L7500 — `veilHeld` reads `keys['KeyF'] ||
  touchVeil` at L6644, mirrored the same way in `qaPlayerState()`'s return  object, so the two inputs are equivalent, not independent) —
...
- `win`/`loss` telemetry events (LUL-1450): `difficulty: Difficulty` field added to
  both `track()` call sites, now in `finishPickup()` (L5965-6025, the live win path as
  of `LUL-2281` -- `arriveHome()`'s L6182-6235 copy is unreachable, kept per Decision 2)
  and `triggerDeath()` (L6236-6279). The `difficulty` module-level variable is in scope
...
- `loss` telemetry event (LUL-2461): `distance_from_home_m` field added --
  ...
  (L6236-6279) fires, computed and stored in `deathDistanceFromHomeM` (module-level,
  set at L6244) rather than recomputed later, since `player.x/z` can move on
...
**What it can do**
- Gate the player's sprint speed (`stepFrame()` at L6597-7436, LUL-2071's extracted per-frame body): `maxSpd = (running ? walk*sprintSpeedMul(staminaCharge) : walk) * ...`, so the player still moves at walk pace when running with zero stamina, but gains speed as stamina refills.
...
`windAssistStartCue()` (L6146) and `windAssistEndCue()` (L6155), edge-triggers on the combined
